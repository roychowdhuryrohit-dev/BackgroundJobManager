...
func CreateUpload(context *gin.Context) {
	...
	sid := signals.GenSID("Jon-Doe", "baselineCSV")
	basepath, ok := config.ConfigMap.Load(config.BaselineCSVPath)
	if !ok {
	...
	go processCSV(sg, sid, filepath)

	context.JSON(http.StatusOK, gin.H{"Job ID": sid})

}

func processCSV(sg *signals.SignalsGroup, sid string, filepath string) {
	var (
		csvFile *os.File
		err     error
	)
...
loop:
	for i := 0; ; {
		...
		default:
			...
			//Print row.
			log.Printf("Job - %s Row - %+q\n", sid, line)
			//Simulate time heavy task for processing each row in CSV.
			time.Sleep(1 * time.Second)
			//Print log for each row iteration.
			i++
			log.Printf("Row %d processed for job %s \n", i, sid)
